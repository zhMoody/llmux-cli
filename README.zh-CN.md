...
**Quota Radar（配额雷达）。** 实时解析上游响应头中的 `x-ratelimit-*` 信息，在仪表盘中展示各账号的 RPM 和 TPM 剩余量。在触发 429 之前，你就清楚地知道还有多少余量。
...
**用量情报。** 全量记录每次请求的延迟、Token 消耗、成功/失败状态和成本。仪表盘以高密度图表呈现，支持按账号和别名下钻分析。
...
- **Accounts** — 启用/禁用账号，设置路由权重，查看实时限速状态
- **Models** — 管理模型别名，将短名称映射到 provider 的模型 ID
