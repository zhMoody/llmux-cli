...
<p align="center">
  ...
  <p align="center">为开发者打造的本地 AI API 网关与多路复用器</p>
</p>
...
作为开发者，你大概同时持有 OpenAI、the provider、Google 等多家平台的账号。每个平台有自己的 SDK、限速策略和接口格式。某个账号触发限速，你就得手动切换、重新配置工具。你想用 the assistant，但需要 Gemini 的吞吐量。你想把 API 访问权限分享给团队，又不想暴露真实密钥。
...
**the provider Ingress（协议跨越）。** 原生支持 the provider 协议。the assistant 等工具可以通过 LLMux 的协议转换层直接调用 Gemini 或 OpenAI 模型，客户端无需任何修改。

**Quota Radar（配额雷达）。** 解析上游响应头中的 `x-ratelimit-*` 信息，在模型卡片上以进度条形式展示 Token 剩余量。每次模型测试后自动更新。需要上游厂商在响应头中返回 `x-ratelimit-remaining-tokens` 等标准字段（OpenAI 支持；智谱、Gemini 等厂商目前不返回此类头信息）。如果厂商未返回这些头，模型卡片只会显示绿色状态点和延迟（秒），不显示进度条。
...
**用量情报。** 全量记录每次请求的延迟、Token 消耗、成功/失败状态。仪表盘以高密度图表呈现，支持按账号、模型和时间范围下钻分析。
...
**5 步完成接入：**
1. **Accounts** — 添加你的 API Key（支持 OpenAI、the provider、Gemini 及自定义端点）
...
- **Dashboard** — Token 用量、延迟分布、请求成功率的实时图表
- **Accounts** — 启用/禁用账号，设置路由权重
- **Models** — 管理模型别名，将短名称映射到 provider 的模型 ID，查看配额剩余量进度条
