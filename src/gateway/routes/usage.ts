...
export function getUsageDetails(req?: Request) {
  try {
    ...
    return Response.json({ byModel, byProvider, byAccount });
  } catch (err: any) {
    return Response.json({ error: err.message }, { status: 500 });
  }
}
...
export function getUsageLogs(req: Request) {
  try {
    ...
    const options = {
      startTime: url.searchParams.get("start") ? Number(url.searchParams.get("start")) : undefined,
      endTime: url.searchParams.get("end") ? Number(url.searchParams.get("end")) : undefined,
